...
Adapted from mjlab (Apache 2.0): https://github.com/mujocolab/mjlab/blob/main/src/mjlab/sim/sim_data.py
...
def _contains_warp_arrays(obj: Any) -> bool:
    ...
    if hasattr(obj, "__dict__"):
        return any(isinstance(getattr(obj, attr), wp.array) for attr in dir(obj) if not attr.startswith("_"))
...
class WarpBridge(Generic[T]):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if _contains_warp_arrays(val):
            wrapped_recurse: WarpBridge[Any] = WarpBridge(val, nworld=self._nworld)
            self._wrapped_cache[name] = wrapped_recurse
            return wrapped_recurse
    ...
    def __repr__(self) -> str:
        ...
        return f"WarpBridge({self._struct!r})"
